...
//        session.delete(student2);
        ...
        //String hqlQuery5 = "DELETE FROM Student07 s  INNER JOIN FETCH Book07 b on s.id = b.s.Student WHERE b.name='Java'";


        // !!! book ismi içinde "Book" geçen student kayıtlarını alalım ( HQL )
        String hqlQuery6 = "SELECT s FROM Student07 s  JOIN s.bookList b WHERE b.name LIKE '%Book%'";
        List<Student07> resultList4 = session.createQuery(hqlQuery6, Student07.class).getResultList();

        for (Student07 student : resultList4) {
            System.out.println(student);
        }
